...
class RecordsApi {
  ...
  Future<List<AwardFinalists>> fetchRecipients(Award award) async {
    final searchUri = Uri.https(baseUrl, '${basePath}trophy/recipient').resolveUri(Uri(queryParameters: {
      'cayenneExp':
    ...
    return await fetch(searchUri, client).then((value) {
      return List<AwardFinalists>.from(getJsonList(['data'], value)
          .map((finalist) => AwardFinalists.fromJson(finalist, award.trophyCategory)));
    });
  }
}
