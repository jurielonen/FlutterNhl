...
class Search {
  ...
  List<PlayerSearch> get getPlayersFiltered {
    if (!needApi && filter != '') {
      return searchValues
          .where((element) =>
              element.fullname.toLowerCase().contains(filter.toLowerCase()))
          .toList();
    }
    return searchValues;
  }
  ...
  factory Search.fromJson(Map<String, dynamic> json) {
    List<PlayerSearch> players = List<PlayerSearch>.from(
        getJsonList(['suggestions'], json)
            .map((search) => PlayerSearch.fromString(search))
            .toList());
    players.sort((a, b) {
      if (a.active && b.active) {
        if (a.rookie && b.rookie) return 0;
        if (a.rookie) return 1;
        if (b.rookie) return -1;
        return 0;
...
class PlayerSearch extends Player {
  ...
  factory PlayerSearch.fromString(String value) {
    ...
    return PlayerSearch(
        player: Player.fromJsonSearchApi(values),
        rookie: int.parse(values[4]) == 1,
        country: values[9],
        jerseyNumber: values[13]);
  }
  String get statusText {
    if (active) return rookie ? 'Rookie' : 'Active';
    return 'Inactive';
  }
}
