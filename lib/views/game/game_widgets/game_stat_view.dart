  ...
  Iterable<Widget> get statListTiles sync* {
    for (String key in homeStats.keys) {
      if (homeStats.containsKey(key) && awayStats.containsKey(key)) {
        yield StatListTile(key, getStatFromMap(key, homeStats),
            getStatFromMap(key, awayStats), homeColor, awayColor);
      }
    }
  }
}
...
class StatListTile extends StatelessWidget {
  ...
  StatListTile(
      this._name, this._home, this._away, this._homeColor, this._awayColor);
      ...
      Row(
        ...
        children: <Widget>[
          ...
          Expanded(
            ...
            child: Center(
                child: Text(
              getColumnTooltip(_name).toUpperCase(),
              style: Styles.statTileText,
            )),
          ),
          Padding(
  ...
  double checkValues() {
    if ((_away == null && _home == null) ||
        (_away == '0' && _home == '0') ||
        (_away == '0.0' && _home == '0.0')) return 0.5;
